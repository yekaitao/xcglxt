...
public class UserInfoUpdate extends JPanel {
	...
	public UserInfoUpdate(User user) {
		...
		setComboxValue(sex,user.getSex());
		...
		setComboxValue(department,user.getDepartment());
		...
		setComboxValue(professionalTitle,user.getProfessionalTitle());
		...
		setComboxValue(mTOA_State,user.getmTOA_State());
		...
		setComboxValue(baseAllowanceState,user.getBaseAllowanceState());
		...
		setComboxValue(dutyAllowanceState,user.getDutyAllowanceState());
		...
		setComboxValue(telAllowanceState,user.getTelAllowanceState());
		...
		setComboxValue(loginAuthority,user.getLoginAuthority());
		...
		creatUserBT.setBounds(143, 383, 123, 29);
		add(creatUserBT);
		
		//更新用户按钮
		JButton btnNewButton = new JButton("\u66F4\u65B0\u7528\u6237");
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(n==1) {
					JOptionPane.showMessageDialog(null, "更新用户成功","提示信息",JOptionPane.OK_OPTION);
				}
				else {
					JOptionPane.showMessageDialog(null, "更新用户失败","提示信息",JOptionPane.WARNING_MESSAGE);
				}
			}
		});
		btnNewButton.setBounds(336, 383, 123, 29);
		add(btnNewButton);
		
		JButton btnNewButton_1 = new JButton("\u66F4\u65B0\u5934\u50CF");
		...
		btnNewButton_1.setBounds(460, 326, 182, 29);
		add(btnNewButton_1);

	}

	//下拉列表选值
	private void setComboxValue(JComboBox cmb, String value) {
		// TODO Auto-generated method stub
		int n = cmb.getItemCount();
		//循环整个下拉列表，匹配哪一个选项是相等的
		for(int i=0;i<n;i++) {
